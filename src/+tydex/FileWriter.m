        ...
        function write(obj, dirpath)
            ...
            for num = 1:numMeas
                ...
                for i=1:numel(nominalParams)
                    try
                        idx = find(strcmp({measurements(num).ModelParameters.Name},nominalParams{i}));
                        data = {
                            widths(1), num2str(measurements(num).ModelParameters(idx).Name),...
                            widths(2), num2str(measurements(num).ModelParameters(idx).Description),...
                            widths(3), num2str(measurements(num).ModelParameters(idx).Unit),...
                            widths(4), num2str(measurements(num).ModelParameters(idx).Value),...
                            };
                        printRow(fileId,data);
                    catch
                    end
