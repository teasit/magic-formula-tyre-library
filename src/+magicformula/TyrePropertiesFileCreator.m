classdef (Hidden) TyrePropertiesFileCreator
    ...
    properties (Access = public)
        FileId
        DisplayConsole logical
    end
    
    methods
        function obj = TyrePropertiesFileCreator(fileId, options)
            arguments
                fileId
                options.DisplayConsole logical = true
            end
            obj.FileId = fileId;
            obj.DisplayConsole = options.DisplayConsole;
        end
        ...
        function printParameter(obj, name, value, comment)
            if obj.DisplayConsole && isempty(value)
